...
zmq::context_t ctx;
zmq::socket_t z_socket(ctx, zmq::socket_type::pub);
...
int main(int argc, char* argv[])
{
  ...
  z_socket.connect("tcp://192.168.0.3:8080");
...
int ReceivePoint(igtl::Socket * socket, igtl::MessageHeader * header)
{
  ...
  if (c & igtl::MessageHeader::UNPACK_BODY) // if CRC check is OK
    ...
    for (int i = 0; i < nElements; i ++)
      ...
      pos_data.push_back(pos[0]);
      pos_data.push_back(pos[1]);
      pos_data.push_back(pos[2]);
      }
      // Send the position data to the desktop
      zmq::message_t posMessage(pos_data);
      z_socket.send(posMessage, zmq::send_flags::dontwait);
    }
